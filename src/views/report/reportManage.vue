        ...
        <v-card>
            <v-card-title class="my-card-title">
                ...
                <v-row style="width:100%">
                    ...
                    <v-col cols="12" sm="4" md="3">
                        <v-btn style="margin:16px;" :loading="downloadloading" :disabled="downloadloading"
                               color="primary" @click="outputExcel()">
                            导出为excel
                            <template v-slot:loader>
                                下载中
                                <span><v-icon class="custom-loader" light>mdi-cached</v-icon></span>
                            </template>
                        </v-btn>
                    </v-col>
                </v-row>
                        ...
                        <v-row>
                            <v-col cols="12" sm="6" md="6">
                                <v-img
                                    ...
                                    @click="
                    viewImg('https://api.twt.edu.cn' + PCData.healthCodeUrl)
                  "
                            ...
                            <v-col cols="12" sm="6" md="6">
                                <v-img
                                    style="cursor: pointer"
                                    @click="
                    viewImg('https://api.twt.edu.cn' + PCData.travelCodeUrl)
                  "
            ...
            <v-dialog v-model="mobileDialog" scrollable max-width="500px">
                <v-card>
                        ...
                        <v-row>
                            ...
                            <v-col cols="12" sm="6" md="6">
                                <v-img
                                    @click="
                    viewImg(
                      'https://api.twt.edu.cn' + mobileShowItem.healthCodeUrl
                    )
                  "
                            ...
                            <v-col cols="12" sm="6" md="6">
                                <v-img
                                    @click="
                    viewImg(
                      'https://api.twt.edu.cn' + mobileShowItem.travelCodeUrl
                    )
                  "
...
import {downloadExcelByDate} from "../../api/user";
...
export default {
    ...
    mounted() {
        ...
        this.outPutDateStartDate = `${nowDate.getFullYear()}-${
            nowDate.getMonth() + 1 < 10
                ? "0" + (nowDate.getMonth() + 1)
                : nowDate.getMonth() + 1
        }-${nowDate.getDate() < 10 ? "0" + nowDate.getDate() : nowDate.getDate()}`;
        this.outPutDateEndDate = `${nowDate.getFullYear()}-${
            nowDate.getMonth() + 1 < 10
                ? "0" + (nowDate.getMonth() + 1)
                : nowDate.getMonth() + 1
        }-${nowDate.getDate() < 10 ? "0" + nowDate.getDate() : nowDate.getDate()}`;
    ...
    methods: {
        ...
        outputExcel() {
            ...
            if (!this.compare(this.outPutDateStartDate, this.outPutDateEndDate)) {
                this.$message.error('导出起始日期大于结束日期')
                return
            }
            this.downloadloading = true
            downloadExcelByDate(this.outPutDateStartDate, this.outPutDateEndDate).then((res) => {
                let link = document.createElement('a'); // 创建a标签
                link.style.display = 'none';
                link.setAttribute('download', '疫情填报.xlsx')//必须要重命名
