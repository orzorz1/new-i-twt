<template>
  <v-container>
    <v-card elevation="1">
      <v-card-title class="text-center py-6"> 健康填报 </v-card-title>
        ...
        <v-tab-item>
          ...
          >
            <v-card-text>
              <div>
                {{
                  result.uploadAt.split("T")[0] +
                  "  " +
                  locationStatus[result.curStatus] +
                  " " +
                  result.temperature +
                  "℃"
                }}
  ...
  methods: {
    ...
    submit() {
      ...
      if (flag) {
        ...
        formData.append(
          "curStatus",
          this.locationStatus.indexOf(this.curStatus)
        );
